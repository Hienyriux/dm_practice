...
with open('fpair_list.csv', 'r') as f:
	f_csv = csv.reader(f)
	for row in f_csv:
		tmp_list = row[0].split('_')
		fname_set.add(tmp_list[0])
		fname_set.add(tmp_list[1])

out_file = open('statistic_train.csv', 'w', newline = '')
...
for i in range(len(train_dir)):
	train_file = os.listdir('../nju-introdm20/train/train/' + train_dir[i])
	for fname in train_file:
		if fname[ : -4] in fname_set:
			continue
		fpath = '../nju-introdm20/train/train/' + train_dir[i] + '/' + fname
